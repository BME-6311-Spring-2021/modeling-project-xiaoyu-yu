...
breed [macrophagesm1 macrophagem1]
breed [macrophagesm2 macrophagem2]
...
to setup
  ...
  ask neutrophils
    ...
    set Neutrophil-Homing-Range 15
    set ShowDebris true
    set showBacteria true
end
...
to Run-Simulation
  ...
  if any? bacteria [
    ;show max(list count bacteria 100)
    let modifierNeu (.03 * min(list count bacteria 200) / (count macrophagesm2 + 1))

    if Neutrophil_Response = "Low" and ticks mod 3 = 0 [create-neutrophils ceiling (.1 * Immune-Cell-Infiltration * modifierNeu) [setxy random-xcor random-ycor setxy pycor pxcor set shape "neutrophil" set age 1]]
    if Neutrophil_Response = "Normal" [create-neutrophils ceiling (1 * Immune-Cell-Infiltration * modifierNeu)[setxy random-xcor random-ycor setxy pycor pxcor set shape "neutrophil" set age 1]]
    if Neutrophil_Response = "High" [create-neutrophils ceiling (2 * Immune-Cell-Infiltration * modifierNeu) [setxy random-xcor random-ycor setxy pycor pxcor set shape "neutrophil" set age 1]]

  ]

    if any? patches with [debris > 0 and ticks > 24] [
    ;let monocyteNum 0
    let debrisNum count patches with [pcolor = brown]
    let modifierMonocyte (0.01 * min(list (count neutrophils * count bacteria * 0.01) 200))
    ;let modifierMonocyte 1 * count neutrophils * 0.1
    let monocyteNum (Immune-Cell-Infiltration * modifierMonocyte)
    let m1Rate count neutrophils * debrisNum * count bacteria * 0.0001 / (count macrophagesm2 + 1)
    let m2Rate (200 * count macrophagesm1 / (count bacteria + 1))
    let m1Percent m1Rate / (m1Rate + m2Rate + 1)
    let m2Percent m2Rate / (m1Rate + m2Rate + 1)

    if Macrophage_Response = "Low" and ticks mod 3 = 0 [create-macrophagesm1 ceiling (.1 * monocyteNum * m1Percent * M1inhibit / 100) [setxy random-xcor random-ycor setxy pycor pxcor set shape "macrophagem1" set age 1]]
    if Macrophage_Response = "Normal" [create-macrophagesm1 ceiling (1 * monocyteNum * m1Percent * M1inhibit / 100)[setxy random-xcor random-ycor setxy pycor pxcor set shape "macrophagem1" set age 1]]
    if Macrophage_Response = "High" [create-macrophagesm1 ceiling (2 * monocyteNum * m1Percent * M1inhibit / 100) [setxy random-xcor random-ycor setxy pycor pxcor set shape "macrophagem1" set age 1]]

    if Macrophage_Response = "Low" and ticks mod 3 = 0 [create-macrophagesm2 ceiling (.1 * monocyteNum * m2Percent * M2inhibit / 100) [setxy random-xcor random-ycor setxy pycor pxcor set shape "macrophagem2" set age 1]]
    if Macrophage_Response = "Normal" [create-macrophagesm2 ceiling (1 * monocyteNum * m2Percent * M2inhibit / 100)[setxy random-xcor random-ycor setxy pycor pxcor set shape "macrophagem2" set age 1]]
    if Macrophage_Response = "High" [create-macrophagesm2 ceiling (2 * monocyteNum * m2Percent * M2inhibit / 100) [setxy random-xcor random-ycor setxy pycor pxcor set shape "macrophagem2" set age 1]]
  ...
  if showBacteria and count bacteria = 0
  [show clock
  set showBacteria false]


  ask neutrophils with [age > 48][if 100 * (age / 96) > random 100 [die ask patch-here [set debris debris + 1]]]
  ask macrophagesm1 with [age > 96][if 100 * (age / 192) > random 100 [die]]
  ask macrophagesm2 with [age > 96][if 100 * (age / 192) > random 100 [die]]
    ...
    if age mod 6 = 0 and age < 48 and count neutrophils-on neighbors = 0
      [
        hatch 1 [
          ...
          [
            if any? neighbors with [not any? neutrophils-here]
...
to proliferate-macrophages                       ; this procedure simulates cell proliferation with contact inhibition for macrophages
  ask macrophagesm1 [
    if age mod 15 = 0 and age < 48 and count macrophagesm1-on neighbors = 0
      [
        hatch 1 [
          let attempted-moves  0
          while [any? other macrophagesm1-here and attempted-moves < 16]
          [
            if any? neighbors with [not any? macrophagesm1-here]
                [move-to one-of neighbors with [not any? macrophagesm1-here]]
            set attempted-moves attempted-moves + 1]
    ...
    if age mod 15 = 0 and age < 48 and count macrophagesm1-on neighbors = 0
      [
        hatch 1 [
          let attempted-moves  0
          while [any? other macrophagesm2-here and attempted-moves < 16]
          [
            if any? neighbors with [not any? macrophagesm2-here]
            [move-to one-of neighbors with [not any? macrophagesm2-here]]
            set attempted-moves attempted-moves + 1]
...
to Leukocyte-Actions
  ...
  ask macrophagesm1 [if any? bacteria-on patch-here
    [ask bacteria-on patch-here [die]]
...
"Macrophagesm1" 1.0 0 -10141563 true "" "plot count macrophagesm1"
"Macrophagesm2" 1.0 0 -7500403 true "" "plot count macrophagesm2"
...
Polygon -1 true false 29 212 21 194
Polygon -16777216 false false 36 197
Polygon -2674135 true false 285 105 210 0 90 0 0 105 15 210 90 300 210 300 300 210 150 150 285 105
Circle -1 true false 159 54 42
...
Polygon -1 true false 29 212 21 194
Polygon -16777216 false false 36 197
Polygon -13840069 true false 285 105 210 0 90 0 0 105 15 210 90 300 210 300 300 210 150 150 285 105
Circle -1 true false 159 54 42
